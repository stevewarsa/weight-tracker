...
export class ButtonCellRenderer implements ICellRendererComp {
    ...
    init(params: any) {
        ...
        buttonLabel = params.label ? params.label : typeof params.getLabel === "function" ? params.getLabel(params) : params.value || "";
        this.button = document.createElement("button");
        this.button.setAttribute("type", "button");
        this.button.className = "btn btn-link my-0 mr-2 p-0";
        this.button.tabIndex = -1;
        this.button.title = params.title ? params.title : typeof params.getTitle === "function" ? params.getTitle(params) : params.value || "";
        this.button.style.textDecoration = params.showUnderline ? "underline" : "none";

        if (params.iconClass) {
            this.button.innerHTML = "<span class='" + params.iconClass + "'>" + buttonLabel + "</span>";
        } else {
